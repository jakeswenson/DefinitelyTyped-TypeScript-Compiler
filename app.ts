...
function getDownloadFolder(user) { return path.join(currentFolder, '../definitelyTypedFiles', user); }
...
module Compilation {
    module TypeScriptCompiler {
        ...
        function getLocationText(location: TypeScript.Location): string {
            return location.fileName() + "(" + (location.line() + 1) + "," + (location.character() + 1) + ")";
        }

        function getFullDiagnosticText(diagnostic: TypeScript.Diagnostic): string {
            var result = "";
        ...
        class ResolverHost implements TypeScript.IReferenceResolverHost {
            public getScriptSnapshot(fileName: string): TypeScript.IScriptSnapshot {
                var text = fs.readFileSync(fileName);
                return TypeScript.ScriptSnapshot.fromString(text.toString());
            }

            public resolveRelativePath(to: string, from: string): string {
            ...
            public getParentDirectory(dir: string): string {
                var p = path.dirname(dir);
                if (p === dir) return null;
                return p;
            }

            public static Current = new ResolverHost();
        }

        function getCompiler(): TypeScript.TypeScriptCompiler {
        ...
        function addFile(
            ...
            fileName: string,
            callback: (compiler: TypeScript.TypeScriptCompiler) => void) {
        ...
        function addFiles(
            ...
            addFile(compiler, files.pop(), compiler => {
        ...
        function runCompiler(fileName, callback) {
            setTimeout(function () {
                ...
                var resolution: TypeScript.ReferenceResolutionResult =
                    TypeScript.ReferenceResolver.resolve([fileName], ResolverHost.Current, true);

                //console.log("File resolution complete", resolution.resolvedFiles.join(', '), resolution.diagnostics.length);

                var files = [];

                resolution.resolvedFiles.forEach(file => {
                    file.referencedFiles.forEach(ref => files.push(ref));
                ...
                if (hasResolutionErrors) {
                    callback({
                        error: resolution.diagnostics.map(diag => diag.text() + "\r\n"),
                ...
                addFiles(compiler, files, compiler => {
                    ...
                    var errors = syntaxDiag
                        .concat(semanticDiag)
                        .map(diag => getFullDiagnosticText(diag));
        ...
        export function getTypeScriptCompiler(files, callback) {
    ...
    export function scheduleCompile(user: string, files: string[], runData: RunData) {
        setTimeout(function () { compile(user, files, runData); }, 1);
    }

    export function compile(user: string, files: string[], runData: RunData) {
        var compileResults = {};
...
module GitHub.DefinitelyTyped {

    function finishDownload(user: string, lastCommit: string, lastUpdate: Date, compilerFiles: string[]) {
    ...
    export function processRepo(github: any, user: string, repo: string) {
