...
function getDownloadFolder(user) {
    return path.join(__dirname, '../definitelyTypedFiles', user);
}
...
function addFile(compiler, fileName, callback) {
    ...
    fs.readFile(fileName, function (err, fileData) {
        ...
        compiler.addFile(fileName, scriptSnapshot, TypeScript.ByteOrderMark.None, 1, true, function (file) {
            throw file;
        });
        callback(compiler);
    });
            ...
            if (exists) {
                fs.readFile(statusFile, function (err, fileJSON) {
                    ...
                    if (lastRunData.lastCommit === latestCommit) {
                        ...
                        setTimeout(function () {
                            compile(info.user, lastRunData.files, lastRunData);
                        }, 1);
                        return;
                    } else {
